...
(function() {
  ...
  function renderCommands(query) {
    ...
    cmdResults.querySelectorAll('.cmd-item').forEach(function(el) {
      el.addEventListener('click', function() {
        ...
        if (action === 'whatsapp') toggleWhatsApp();
  ...
  function toggleWhatsApp() {
    ...
    if (window.AIA && waPopup.classList.contains('active')) {
      window.AIA.Audit.log('help_widget_opened', {});
    }
  }

  if (waTrigger) waTrigger.addEventListener('click', toggleWhatsApp);
  if (waPopupClose) waPopupClose.addEventListener('click', function() { waPopup.classList.remove('active'); });
  ...
  document.addEventListener('click', function(e) {
    ...
    if (e.target.closest('.user-dropdown, .notif-panel, .activity-panel, .wa-popup')) return;
    ...
    if (!e.target.closest('.wa-trigger, .wa-popup')) waPopup.classList.remove('active');
  });
